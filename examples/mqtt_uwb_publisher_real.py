...
def on_mqtt_connect(client, userdata, flags, rc):
    log_verbose(f"MQTT connect callback: client_id={client._client_id.decode() if hasattr(client._client_id, 'decode') else client._client_id}, flags={flags}, rc={rc}")
    if rc == 0:
        print(f"Connected to MQTT broker {args.mqtt_broker}:{args.mqtt_port} with SSL (Client ID: {args.mqtt_client_id})")
        log_verbose("MQTT SSL connection successful")
    else:
        error_messages = {
...
def setup_mqtt():
    ...
    try:
        log_verbose(f"Creating MQTT client instance with client ID: {args.mqtt_client_id}")
        mqtt_client = mqtt.Client(client_id=args.mqtt_client_id)
        ...
        # Connect to broker
        log_verbose(f"Attempting to connect to {args.mqtt_broker}:{args.mqtt_port} with client ID '{args.mqtt_client_id}' using SSL")
        ...
        print(f"MQTT client configured for {args.mqtt_broker}:{args.mqtt_port} (Client ID: {args.mqtt_client_id}) with SSL")
        return mqtt_client
...
def print_list(results):
    ...
    # Always display data immediately
    print(row)
    
    # Publish to MQTT with rate limiting (separate from display)
...
# Initialize MQTT
log_verbose(f"Initializing MQTT client with ID: {args.mqtt_client_id}")
